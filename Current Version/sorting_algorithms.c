...
void	sort_over_five(t_stack **stack_a, t_stack **stack_b)
{
	...
	//errrrrooooooooooo aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
	...
	temp = (*stack_a)->prev;
	lowest_index = ft_find_index(stack_a, 'L');
	while (*stack_b != NULL)
	{
		if (temp->index > (*stack_b)->index)
			do_pa(stack_b, stack_a);
		else if (temp->index > (*stack_b)->index)
		{
			if (temp->index != ft_find_index(stack_a, 'H'))
			{
				do_rra(stack_a);
				temp = temp->prev;
			}
	...
	printf("temp: %d lowest %d\n", temp->index, lowest_index);
